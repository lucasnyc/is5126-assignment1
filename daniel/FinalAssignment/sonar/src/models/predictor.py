...
class FreightPredictor:
    ...
    def explain_edge(
        ...
        Note: year 2022+ predictions use the TFT model (see tft_predictor.py).
        """
        if year >= 2022:
            raise ValueError(
                f"Year {year} predictions are generated by the TFT model. "
                "Use tft_predictions_2022.parquet (loaded by 04_model_insights.py) instead."
            )
        edges = load_edges()
        mask = (
            (edges["origin"] == origin)
...
def load_edges() -> pd.DataFrame:
    ...
    After running generate_tft_predictions.py the returned DataFrame also includes
    year=2022 rows (TFT forecasts) with additional columns:
        q10, q20, q30, q50, q70, q80, q90  — quantile uncertainty bands
        model_source                         — "observed", "xgb", or "tft"
    """
